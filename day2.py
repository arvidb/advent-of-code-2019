...
def run_program(program, verb, noun):
	global idx
	idx = 0
	...
	while idx < len(program):
		...
		if op == '99':
			...
			op1 = int(program[next()])
			op2 = int(program[next()])
			out = int(program[next()])

			if op == '1':
				program[out] = int(program[op1]) + int(program[op2])
			else:
				program[out] = int(program[op1]) * int(program[op2])
	...
	result = program[0]
	if result == 19690720:
		print("Found", 100*noun+verb)

for verb in range(0, 100):
	for noun in range(0, 100):		
		try:
			run_program(copy.copy(program), verb, noun)
		except:
			pass
